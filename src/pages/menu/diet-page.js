...
const Diet = () => {
  ...
  const fetchDietFoods = async () => {
    const results = await getFoodsByCategory('Diet');
    setFoods(results);
  ...
  const handleSearch = async () => {
    const query = searchTerm.trim().toLowerCase();
      ...
      const dietFoods = await getFoodsByCategory('Diet');
      const filtered = dietFoods.filter(item =>
  ...
  const totalPages = Array.isArray(filteredFoods) ? Math.ceil(filteredFoods.length / itemsPage) : 0;
  const indexOfLastItem = currentPage * itemsPage;
  const indexOfFirstItem = indexOfLastItem - itemsPage;
  const currentItems = Array.isArray(filteredFoods)
    ? filteredFoods.slice(indexOfFirstItem, indexOfLastItem)
    : [];
    ...
    <div className="foodlist-container">
      <Container fluid className="p-0">
        <Row className="text-center mb-4 mt-3">
          <Col>
            <h1 className="foodlist-title">Diet Menu</h1>
          </Col>
        </Row>
