...
const Maintenance = () => {
  ...
  const fetchMaintenanceFoods = async () => {
    try {
      const results = await getFoodsByCategory('Maintenance');
      setFoods(results);
  ...
  const handleSearch = async () => {
    const query = searchTerm.trim().toLowerCase();
        ...
        const maintenanceFoods = await getFoodsByCategory('Maintenance');
        const filtered = maintenanceFoods.filter(item =>
  ...
  const totalPages = Math.ceil((filteredFoods?.length || 0) / itemsPage);
